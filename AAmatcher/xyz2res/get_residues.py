...
def write_residues(g, get_residue=get_residue):
    ...
    for n in c_alphas:
        ...
        # predict residue
        residue = get_residue(res_subgraph)
    ...
    if any(g.ndata["pred_residue"]<0):
        for i in range(2):
            ...
            residue = get_residue(dgl.node_subgraph(g, cap))
            res_idx = RESIDUES.index(residue)
            ...
            if residue == "NME":
                ...
                raise RuntimeError(f"Encountered unallowed cap residue {residue}")
