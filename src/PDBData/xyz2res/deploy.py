...
from PDBData.xyz2res.model_ import Representation

from PDBData.xyz2res.constants import MAX_ELEMENT
...
def xyz2res(xyz:np.ndarray, atom_numbers:np.ndarray, debug:bool=False):
    ...
    if debug:
        print(f"found {g.ndata['c_alpha'].sum().item()} c alphas")
        print(f"xyz is\n{xyz}")
        from PDBData.utils.draw_mol import draw_mol
